...
function HealthBadge:OnUpdate(dt)
	local down = self.owner.components.temperature:IsOverheating() or self.owner.components.temperature:IsFreezing() or
	self.owner.components.hunger:IsStarving() or self.owner.components.health.takingfiredamage
	...
	-- 改动了
	anim = (GetPlayer().components.arcueidstate.abilityselected == 1) and "arrow_loop_increase_more" or anim
	----
