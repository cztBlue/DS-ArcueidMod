...
local function InitEnvelope_acid()
	if EnvelopeManager and not init then
		init = true
		EnvelopeManager:AddColourEnvelope(
			colour_envelope_name,
			{ { 0, IntColour(218, 226, 83, 255) },
				{ 1, IntColour(218, 226, 83, 255) },
			})
...
local function fn(Sim)
	...
	local isacid = GetPlayer().components.arcueidstate:GetErosionPercent() >= .5

	if GetPlayer() and GetPlayer().prefab == "arcueid" and isacid then
		InitEnvelope_acid()
	else
		InitEnvelope()
	end
		...
		if GetWorld().Map ~= nil then
			while inst.num_splashes_to_emit > 0 do
				...
				if GetWorld().Map:GetTileAtPoint(x, y, z) ~= GROUND.IMPASSABLE then
					local raindrop
					if GetPlayer() and GetPlayer().prefab == "arcueid" and isacid then
						raindrop = SpawnPrefab("arcueid_acidraindrop")
					else
						raindrop = SpawnPrefab("raindrop")
					end
